...
export const Dialog = defineComponent({ props: ['modelValue'], setup(props, { slots }) { return () => props.modelValue ? h('div', { class: 'fixed inset-0 bg-black/50 flex items-center justify-center' }, h('div', { class: 'bg-white p-6 rounded-lg' }, slots.default?.() || slots['body-content']?.())) : null; } });
